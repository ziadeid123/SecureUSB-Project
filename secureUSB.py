...
def encrypt_file(file_path, key, algorithm="AES"):
    ...
    if algorithm == "AES":
        cipher = Cipher(algorithms.AES(key), modes.CFB(b"16BytesIVVector"))
    elif algorithm == "ChaCha20":
        cipher = Cipher(algorithms.ChaCha20(key, b"12BytesIVVVVVV"), mode=None)
    else:
        ...
        try:
            ...
            if user_password == stored_password:
                ...
                if choice == "1":
                    ...
                    algorithm = input("Choose encryption method (AES/ChaCha20): ")
                    encrypt_file(file_path, key, algorithm=algorithm)
                elif choice == "2":
                    file_path = input("Enter the path of the file to decrypt: ")
                    algorithm = input("Choose decryption method (AES/ChaCha20): ")
                    decrypt_file(file_path, key, algorithm=algorithm)
                else:
